...
class AuthController extends Controller {
    ...
    public function login(Request $request) {
        try {
            return view('administration.auth.login', $this->responceData);
        } catch (Exception $ex) {
        ...
        try {
            ...
            if ($validator->fails()) :
                ...
                if ($userExists == 0):
                    ...
                    return redirect('administration/login')->withInput();
                else:
                    $user = User::where('email', $email)->first();
                    
                    /* login porocess */
                    if (Auth::guard('administration')->attempt(['email' => $email, 'password' => $password, 'status' => 1, 'user_type' => 6])) :
                        $request->session()->flash('success', 'Welcome ' . Auth::guard('administration')->user()->first_name . ' ' . Auth::guard('administration')->user()->last_name . '.');
    ...
    public function forgotPassword(Request $request) {
        try {
            return view('administration.auth.forgot-password', $this->responceData);
        } catch (Exception $ex) {
            CommonFunction::generateErrorLog($ex, $request->all());
            $request->session()->flash('error', 'An error occurred.');
            return redirect('/administration');
        ...
        try {
            ...
            if ($validator->fails()) :
                ...
                if ($count == 0):
                    ...
                    return redirect('administration/forgot-password');
                else:
