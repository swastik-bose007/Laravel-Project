...
class FarmerController extends Controller {
    ...
    public function userList(Request $request) {
        try {
            $query = User::with([
                                ...
                                ->where('created_by', '=', Auth::guard('bazarmandi')->user()->id);
            // for search
            ...
            return view('bazarmandi.farmers-management.list', $this->responceData);
        } catch (Exception $ex) {
    ...
    public function userCreate(Request $request) {
        try {
            $this->responceData['listData'] = User::with([
                                                    'getRoles',
                                                    'getStatus'
                                                ])
                                                ->where('id', '=', Auth::guard('bazarmandi')->user()->id)
                                                ->orderBy('id', 'desc')
                                                ->paginate(env('PAGINATE'));
            $this->responceData['userRoles'] = UserRoles::where('status', 1)->get();
            $this->responceData['statusList'] = Status::get();

            return view('bazarmandi.farmers-management.add', $this->responceData);
        } catch (Exception $ex) {
            CommonFunction::generateErrorLog($ex, $request->all());
            $request->session()->flash('error', 'An error occurred.');
            return redirect('/bazarmandi/dashboard');
    ...
    public function userCreateProcess(Request $request) {
        ...
        try {
            ...
            $validator = Validator::make($request->all(), [
                                ...
                                'pincode'            => 'required|string|max:60',
                                'email'              => 'required|email|unique:tk_users,email|max:60',
                                'phone_no'           => 'required|numeric|unique:tk_users,phone',
                                'adhar_no'           => 'required|numeric|unique:tk_users,adhar_no',
                                'pan_no'             => 'required|numeric|unique:tk_users,pan_no',
                                'password'           => 'required|confirmed|min:6|max:20',
                                'bazar_mandi_id'     => 'required|numeric|exists:tk_users,id',
                            ]);
            ...
            if ($validator->fails()) :
                ...
                $user->area                              = ucfirst(strtolower($request->area));
                $user->district                          = ucfirst(strtolower($request->district));
                ...
                $user->user_type                         = 3;
                $user->created_by                        = $request->bazar_mandi_id;
    ...
    public function userEdit($slug, Request $request) {
        try {
            ...
            if($exists):
                ...
                return view('bazarmandi.farmers-management.edit', $this->responceData);
            else:
    ...
    public function userEditProcess(Request $request) {
        try {
            ...
            $phoneNumberExsist = User::where('slug', '!=', $request->slug)
