...
class ProduceController extends Controller {
    ...
    public function produceList(Request $request) {
        try {
            ...
            return view('bazarmandi.produce.list', $this->responceData);
        } catch (Exception $ex) {
    ...
    public function produceCreate(Request $request) {
        try {
            $this->responceData['produceNameList']  = ProduceName::with(['getCategory'])
                                                        ->orderBy('id', 'desc')
                                                        ->where('status', 1)
                                                        ->get();
            ...
            return view('bazarmandi.produce.add', $this->responceData);
        } catch (Exception $ex) {
            CommonFunction::generateErrorLog($ex, $request->all());
            $request->session()->flash('error', 'An error occurred.');
            return redirect('bazarmandi/dashboard');
    ...
    public function produceCreateProcess(Request $request) {
        try {
            ...
            if ($validator->fails()) :
                ...
                $produce = new Produce;
                $produce->created_by            = Auth::guard('bazarmandi')->user()->id;
                ...
                //for getting produce name tags data.
                    ...
                    $tagUser=User::Where('id',Auth::guard('bazarmandi')->user()->id)->get();
    ...
    public function produceEdit($slug, Request $request) {
        try {
            ...
            if($produce != null):
                ...
                return view('bazarmandi.produce.edit', $this->responceData);
            else:
