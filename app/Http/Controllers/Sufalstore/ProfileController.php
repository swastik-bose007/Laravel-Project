...
class ProfileController extends Controller {
    ...
    public function index(Request $request) {
        $this->responceData['produceCount'] = Produce::where('created_by', Auth::guard('sufalstore')->user()->id)->count();

        return view('sufalstore.profile.edit', $this->responceData);
    }
    ...
    public function profileUpdate(Request $request) {
        ...
        try {
            ...
            if ($validator->fails()) :
                ...
                $phoneNumberExsist = User::where('id', '!=', Auth::guard('sufalstore')->user()->id)
                ...
                $user = Auth::guard('sufalstore')->user();
                ...
                switch ($countarr){
                        case 1:
                            $user->bazar_mandi_name = ucfirst(strtolower($user->first_name))." "."Sufal Store";
                        break;
    ...
    public function changePassword(Request $request) {
        try {
            ...
            if ($validator->fails()) :
                ...
                if (!(Hash::check($request->current_password, Auth::guard('sufalstore')->user()->password))) :
    ...
    public function profileImageUpdate($slug,Request $request) {
        try {
            ...
            $this->responceData['profileImage']= ProfileImage::where('user_id', Auth::guard('sufalstore')->user()->id)->get();

            return view('sufalstore.profile.profile-image-add', $this->responceData);
    ...
    public function setProfilePicture($slug, Request $request) {
        try {
            $user=User::where('id', Auth::guard('sufalstore')->user()->id)->get();
            ...
            $request->session()->flash('success', 'Successfully Updated.');
                    return redirect('sufalstore/profile');
        } catch (Exception $ex) {
            CommonFunction::generateErrorLog($ex, $request->all());
            $request->session()->flash('error', 'An error occurred.');
            return redirect()->back();
    ...
    public function setDemoProfilePicture(Request $request) {
     ...
     public function deleteProfilePicture($id,Request $request) {
        ...
        $user=User::where('id', Auth::guard('sufalstore')->user()->id)->get();
        $profilePicture=User::where('id', Auth::guard('sufalstore')->user()->id)->get("profile_picture_url");


        $imageExists=count(ProfileImage::where('user_id',Auth::guard('sufalstore')->user()->id)->get());
