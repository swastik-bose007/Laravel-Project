                ...
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item">
                            <a href="{{ url('bazarmandi/farmers-management/farmer-produce/list/'. $farmerSlug) }}">Farmer Produce List</a>
                        </li>
            ...
            <div class="row">
                <div class="col-sm-12">
                    <div class="card card-primary">
                        ...
                        <!-- /.card-header -->
                        <!-- form start -->
                        <form id="createForm" name="createForm" autocomplete="off" enctype="multipart/form-data"
                            method="post" action="{{ url('bazarmandi/farmers-management/farmer-produce/create') }}">

                            {{ csrf_field() }}

                            <input type="hidden" value="{{ $farmerSlug }}" name="farmer_slug">
                                ...
                                <div class="row">
                                    <div class="col-sm-4">
                                        <div class="form-group">
                                            ...
                                            <select name="produce_name_id" id="produce_name_id" class="form-control">
                                                ...
                                                <option value="{{ $produceName->id }}"
                                                    {{ old('produce_name_id') == $produceName->id ? 'selected' : ''}}
                                                    data-slug="{{ $produceName->slug }}"
                                                    data-category="{{ $produceName->getCategory->name }}">
                                                    {{ $produceName->name }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
                                    ...
                                    <div class="col-sm-4" id="produce_variant_view">
                                        <div class="form-group">
                                            ...
                                            <select name="produce_variant_id" id="produce_variant_id"
                                                class="form-control">
...
$(document).ready(function() {
    ...
    $("#produce_name_id").change(function() {
        ...
        if (typeof categoryName == "undefined") {
            ...
            $.ajax({
                type: 'GET',
                url: '<?php echo url("bazarmandi/farmers-management/farmer-produce/get-produce-name-variant"); ?>/' +
                    slug,
                data: '',
                success: function(data) {
                    ...
                    if (data.data.get_variant.length > 0) {
                        ...
                        for (let loop = 0; loop < data.data.get_variant.length; loop++) {
                            variantHtml += '<option value="' + data.data.get_variant[loop]
                                .id + '">' + data.data.get_variant[loop].name + '</option>';
                        }
